...
const transparentPages = ["/", "/login", "/signup", "/pollachi", "/palani", "/admin/login"];
const isPlacePage = pathname.startsWith("/place/");
const isTransparentPage =
  transparentPages.includes(pathname) || isPlacePage;
    ...
    <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-900 shadow-lg md:hidden">
      {LINKS.map(l => (
        <Link key={l.href} href={l.href} onClick={() => setMenuOpen(false)}
          className="block px-6 py-4 text-sm hover:bg-gray-50 dark:hover:bg-gray-800">
          {l.label}
        </Link>
      ))}

      <Link href="/admin/login" onClick={() => setMenuOpen(false)}
        className="block px-6 py-4 text-sm font-semibold text-amber-600">
        Admin Panel
      </Link>
    </div>
  )}
